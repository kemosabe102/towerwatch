...
def service_restarted(loki, *, version: str, build_date: str, platform: str) -> None:
    loki.push("WARN", "Service restarted", {
...
def service_started(loki, *, log_level: str, platform: str) -> None:
    loki.push("INFO", "Service started", {
...
def connection_down(loki) -> None:
    loki.push("ERROR", "All targets unreachable", {
        "event": config.LOG_EVENT_CONN_DOWN,
    })


def connection_restored(loki, *, down_duration_s: int) -> None:
    loki.log_and_push("INFO", f"Connection restored after {down_duration_s}s",
...
def outage_recorded(loki, *, gap_seconds: int, reason: str, version: str) -> None:
    ...
    loki.push("WARN", f"Outage recorded: {gap_seconds}s ({gap_min} min)", {
...
def service_heartbeat(loki, *, uptime_h: float) -> None:
    loki.push("WARN", "Service heartbeat", {
...
def partition_missing(loki, *, path: str) -> None:
    loki.push("WARN", f"Data partition not detected at {path}", {
        "event": config.LOG_EVENT_PARTITION_MISSING,
        "path": path,
    })


def metrics_push_failed(loki, *, http_status: int = None, error: str = None) -> None:
    extra = {"event": config.LOG_EVENT_METRICS_PUSH_FAIL}
...
def annotation_failed(loki, *, http_status: int = None, error: str = None) -> None:
    extra = {"event": config.LOG_EVENT_ANNOTATION_FAILED}
    if http_status is not None:
        extra["http_status"] = http_status
    if error is not None:
        extra["error"] = error
...
def log_buffer_flushed(loki, *, count: int) -> None:
    loki.push("WARN", f"Log buffer flushed: {count} entries", {
