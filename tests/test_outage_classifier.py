...
from towerwatch.startup import classify_outage, OutageKind
...
def test_short_gap_no_annotation():
    ...
    result = classify_outage(now=now, last_push_ts=last_push,
                             last_alive_ts=None, gap_threshold_s=GAP)
...
def test_long_gap_alive_recent_is_network_unreachable():
    ...
    kind, gap = classify_outage(now=now, last_push_ts=last_push,
                                last_alive_ts=last_alive, gap_threshold_s=GAP)
...
def test_missing_last_push_no_annotation():
    result = classify_outage(now=1_700_000_000.0, last_push_ts=None,
                             last_alive_ts=None, gap_threshold_s=GAP)
    assert result is None
