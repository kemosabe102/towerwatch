...
- **Failure-classification probes are intentional, not redundant.** `dns_resolve_ms_gateway`
  (in `collect_probes`) probes the *gateway's own* resolver — do NOT remove it as "duplicate
  DNS"; a hung gateway resolver is invisible if only public resolvers are probed. Its field
  name is stable (`_gateway`), not IP-derived, so the series doesn't churn on gateway change.
  The egress-IP probe (`probes/egress.py` + `handle_egress_check`) exists to detect LTE
  failover via an egress-IP *change* (the change-event is the detector; `egress_cgnat` is a
  secondary field, NOT the detector — Cloudflare reports the post-NAT public IP). Both are
  all-sites, config-gated (`EGRESS_IP_CHECK_ENABLED`). See `docs/ullrich-gateway-ubc1340.md`.
- **Gateway IP is resolved at *runtime* by `net.GatewayResolver`, not frozen at import.** `config.GATEWAY_IP` / `PROBE_TARGETS` / `M6_ADMIN_URL` are only the boot-time snapshot (kept for the pinning tests in `test_config_gateway.py` + startup wiring). Per-tick code reads `ctx.gateway_resolver.current()` — the ping gateway target, `poll_gateway(ip=...)`, and `poll_m6_signal(ip=...)` all take the live IP. This retired a real outage: a stale deployed `credentials.py` without `GATEWAY_IP_OVERRIDE` let import-time discovery freeze on the `192.168.1.1` fallback during a boot race, producing false 100% gateway loss + a dead M6 scrape (July 2026 standstill). The resolver re-resolves every `DEFAULT_RERESOLVE_INTERVAL_S` (300s), retries at startup (`resolve_with_retry`, so a boot race can't freeze the fallback), emits `gateway_ip` on `build_info` **updating on re-resolution** (so the dashboard shows which IP is probed — do NOT make this startup-only, that recreates the silent failure), and pushes a `gateway_ip_changed` Loki event on change. An override short-circuits all of this. **Don't** revert to a single import-time `GATEWAY_IP` — that's the bug.
- **Data budget is a hard constraint, not a guideline.** Per-site allotment is **30 GB/month**. The always-on probes are ~230 MB/month. The Cloudflare adaptive throughput probe runs at a per-site cadence (`CLOUDFLARE_THROUGHPUT_TESTS_PER_DAY_OVERRIDE` in `credentials.<site>.py`); typical settings are 1×/day on home gigabit, 3×/day on cellular paired with `CLOUDFLARE_THROUGHPUT_WINDOWS_OVERRIDE = [(6,10),(11,14),(17,21)]` for morning/midday/evening sampling. Cloudflare's adaptive ramp sizes individual transfers to measured speed, so per-site byte-cap tuning is rarely needed; `CLOUDFLARE_THROUGHPUT_MAX_TOTAL_BYTES_OVERRIDE` / `CLOUDFLARE_UPLOAD_MAX_TOTAL_BYTES_OVERRIDE` remain available as escape hatches. The dashboard's "Speedtest Data (7d)" stat surfaces actual usage — green ≤6 GB, yellow 6–7 GB, red >7 GB (≈ weekly slice of the monthly budget). Any change that pushes towards the budget ceiling must be justified.
...
- **Dashboard sync uses a *stack service-account* token, not an Access Policy token.** `grafana/*.json` is pushed to Grafana Cloud by the `Sync Dashboards` GitHub Actions workflow (`scripts/sync_dashboards.py`, overwrites in place by `uid`). The token is `GRAFANA_SA_TOKEN` (GitHub repo secret) — minted under the *stack* (`towerwatch.grafana.net` → Administration → Service accounts, Editor role, `glsa_...`). Access Policy tokens (grafana.com, `metrics`/`logs` scopes) **cannot write dashboards** — that's the trap. The dashboard JSON is stored in *export* format (`__inputs` + `${DS_PROMETHEUS}` placeholder); `sync_dashboards.py` resolves the placeholder to `grafanacloud-towerwatch-prom` and strips export-only keys before pushing. Dashboard `uid`s (`towerwatch-main`, `towerwatch-compare`) must stay stable — a blank `uid` creates a duplicate on every push. See [`docs/dashboard-sync.md`](docs/dashboard-sync.md).
